...
function generateShuffledOrder(): {
  questionOrder: number[];  // 셔플된 문항 인덱스 (0-74)
  swappedOptions: Record<number, boolean>;  // 문항 id -> A/B 스왑 여부
} {
  const questionOrder = shuffleArray(Array.from({ length: QUESTION_POOL.length }, (_, i) => i));
  const swappedOptions: Record<number, boolean> = {};

  QUESTION_POOL.forEach(q => {
    swappedOptions[q.id] = Math.random() < 0.5;
  });

  return { questionOrder, swappedOptions };
}
...
export function usePilotSurvey(options: UsePilotSurveyOptions = {}): UsePilotSurveyReturn {
  ...
  const [shuffledOrder, setShuffledOrder] = useState<{
    ...
    swappedOptions: Record<number, boolean>;
  } | null>(null);
  ...
  // 셔플된 순서에 따른 현재 RIASEC 문항 (보기도 스왑 적용)
  const currentRiasecQuestion = useMemo(() => {
    if (!shuffledOrder) return null;
    const originalIndex = shuffledOrder.questionOrder[riasecIndex];
    if (originalIndex === undefined) return null;

    const originalQuestion = QUESTION_POOL[originalIndex];
    if (!originalQuestion) return null;

    const isSwapped = shuffledOrder.swappedOptions[originalQuestion.id];

    if (isSwapped) {
      // A/B 스왑
      return {
        ...originalQuestion,
        A: originalQuestion.B,
        B: originalQuestion.A,
        _isSwapped: true,  // 스왑 여부 표시 (내부용)
      };
    }

    return { ...originalQuestion, _isSwapped: false };
    ...
    if (saved) {
      try {
        ...
        if (parsed.phase === 'riasec') {
          ...
          // 저장된 셔플 순서 복원
          if (parsed.shuffledOrder?.questionOrder && parsed.shuffledOrder?.swappedOptions) {
  ...
  const answerRiasecQuestion = useCallback((choice: 'A' | 'B') => {
    if (!currentRiasecQuestion || !shuffledOrder) return;

    const questionId = currentRiasecQuestion.id;
    const isSwapped = shuffledOrder.swappedOptions[questionId];

    // 스왑된 경우 원래 선택으로 변환 (화면에서 A 선택 → 실제 B 저장)
    const actualChoice: 'A' | 'B' = isSwapped
      ? (choice === 'A' ? 'B' : 'A')
      : choice;
  ...
  }, [currentRiasecQuestion, shuffledOrder, riasecIndex, riasecAnswers, calculateRiasecScores, saveRiasecResult]);
