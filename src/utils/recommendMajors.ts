...
export function recommendMajors(
  ...
  const scoredMajorsRaw = MAJORS
    ...
    .filter((major): major is (typeof MAJORS[0] & { rawScore: number; signature: string; reasons: string[]; clusterBonus: number }) => Boolean(major))
    .sort((a, b) => {
  ...
  // 🆕 상대 점수 계산: 1위를 100%로, 나머지는 상대적 비율로 표시
  // 차이를 5배 증폭하여 미세한 차이도 구분되게 (0.01 차이 → 5% 차이)
  const topRawScore = scoredMajorsRaw[0]?.rawScore || 1;
  const SCORE_AMPLIFIER = 5;
  const scoredMajors = scoredMajorsRaw.map((major, index) => {
    ...
    if (index === 0) {
      ...
      // 1위 대비 차이를 증폭
      const diff = topRawScore - major.rawScore;
      const amplifiedDiff = diff * SCORE_AMPLIFIER;
      matchScore = Math.max(0, Math.round((1 - amplifiedDiff) * 100));
    }

    const { rawScore, ...rest } = major;
    return {
      ...rest,
      matchScore
    } as RecommendedMajor;
